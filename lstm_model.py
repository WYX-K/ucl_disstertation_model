...
class LSTM(nn.Module):
    def __init__(self, vocab_size, region_embedding_dim, time_embedding_dim, hidden_size, output_size, num_layers=1, bidirectional=False, dropout=0.5):
    ...
    def forward(self, region_sequences, time_sequences, original_length):
        ...
        if self.output_size == 1:
            out = self.binary_classifier(hidden[-1])
        else:
            out = self.multi_classifier(hidden[-1])
        return out
